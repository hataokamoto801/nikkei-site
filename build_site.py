# -*- coding: utf-8 -*-
"""
日経平均 理論株価サイト 自動生成スクリプト v4
================================================================
【v4の変更点】
PBR別の理論株価（1.2倍～2.2倍）を追加。PBR2.0倍に過熱目安マークを表示。
BPS = 日経平均終値 ÷ PBR(加重平均) で算出。
...
PER_MIN = 11
PER_MAX = 21
PBR_MIN10 = 12   # PBR下限×10（=1.2倍）
PBR_MAX10 = 22   # PBR上限×10（=2.2倍）
PBR_HOT10 = 20   # 過熱目安×10（=2.0倍）
...
<div class="wrap">
  ...
  <div class="stats">
    ...
    <div class="stat"><div class="l">PBR</div><div class="v">$pbr 倍</div></div>
    <div class="stat"><div class="l">BPS</div><div class="v">$bps</div></div>
  </div>
...
$rows
  ...
  <div class="sec">
  <h2>PBR別 理論株価</h2>
  <p class="note">理論株価 ＝ BPS $bps 円 × PBR倍率（加重平均ベース）。PBR2.0倍は過熱の目安とされ、その水準で調整が入る場面があります。</p>
...
$pbr_rows
  ...
  <footer>
    データ出所: $source。EPSは「日経平均÷PER(加重平均)」、BPSは「日経平均÷PBR(加重平均)」で算出。
...
def build_html(rec):
    ...
    if pbr_now and bps:
        marker2 = False
        for t10 in range(PBR_MAX10, PBR_MIN10 - 1, -1):
            p = t10 / 10
            price = bps * p
            if (not marker2) and pbr_now >= p:
                pbr_rows.append(
                    f'      <tr class="now"><td>現在 {pbr_now:.2f}倍</td>'
                    f'<td class="price">¥{nikkei:,.0f}</td><td class="diff">―</td></tr>'
                )
                marker2 = True
            diff = price - nikkei
            ...
            if t10 == PBR_HOT10:
                cls += " hot"
            sign = "+" if diff >= 0 else ""
            hot = '<span class="hotmark">⚠ 過熱目安</span>' if t10 == PBR_HOT10 else ""
            pbr_rows.append(
                f'      <tr class="{cls}"><td>{p:.1f}倍{hot}</td>'
                f'<td class="price">¥{price:,.0f}</td>'
                f'<td class="diff">{sign}{diff:,.0f}円（{sign}{pct:.1f}%）</td></tr>'
            )
        if not marker2:
        ...
        pbr_str = f"{pbr_now:.2f}"
        bps_str = f"{bps:,.2f}"
    else:
        pbr_rows.append('      <tr><td colspan="3">PBRデータが取得できませんでした</td></tr>')
        pbr_str, bps_str = "―", "―"
